...
.不支持下标索引  
常用操作 - 修改  
.添加新元素：set.add(value ) 集合本身会改变  
.删除元素：set.remove(value) 集合本身会改变  
.随机取出一个元素：set.pop() 集合本身会改变 （因为 set 无序的，不支持下标索引，所以随机取出，元素被移除,并返回）  
.清空集合：set.clear() 集合本身会改变  
.集合差集：set1.difference(set2) 返回两个集合中不同的元素 (得到新集合，不影响原集合)  
.消除差集：set1.difference_update(set2) 对比集合 1 和集合 2，在集合 1 内，删除和集合 2 相同的元素 (集合 set1 被修改，集合 2 不变)  
.集合合并：set1.union(set2) 返回两个集合的并集 (得到新集合，不影响原集合)  
.统计集合中元素个数：len(set)
...
排序  
.sorted(容器,[reverse=true]) 方法: 对列表进行排序  
.返回 list 类型
...
.1 文件编码  
计算机只能识别 0 和 1，所以无法直接处理文本文件，需要将文本文件转换成计算机能够识的二进制  
.utf-8 编码：使用 1 个字节表示英文字母，2 个字节表示汉字  
.gbk 编码：使用 1 个字节表示英文字母，2 个字节表示汉字  
.gb2312 编码：使用 1 个字节表示英文字母，2 个字节表示汉字  
.gb18030 编码：使用 1 个字节表示英文字母，2 个字节表示汉字

2.什么是文件  
内存中存放的数据在计算机关机后就会消失。要长久保存数据，就要使用硬盘，光盘，U 盘等设备  
为了便于数据的管理和索引，引入了文件的概念

.一篇文章，一段视频，一个可执行程序，都可以被保存为一个文件，并赋予一个文件名。  
.操作系统以文件为单位管理磁盘的数据。一般来说，文件可以分为  
.文本文件，视频文件，音频文件，可执行文件，图像文件等多种类别

3.文件操作
.1.打开文件
.open(file, mode='r', encoding=None) ：打开已存在的文件/或者创建新文件，返回文件对象
.file: 文件路径和文件名，字符串类型
.mode: 打开文件的模式（r只读，w写入，a追加等），字符串类型
.encoding: 文件编码，字符串类型（推荐 utf-8）
...
一.了解异常  
.什么是异常  
当检测到一个错误时，Python解释器就无法继续执行了，反而出现了一些错误的提示，这就是所谓的“异常”，也就是
我们常说的BUG
...
.\_\_name\_\_是python内置的变量，表示当前模块名  
#当模块被直接运行时，\_\_name\_\_的值为\_\_main\*\*

\_\_all\_\_如果模块文件有\_\_all\_\_变量，当使用from xxx import \* 导入时，只能导入这个列表中的元素
